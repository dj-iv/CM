document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATA ---
    // This is no longer a constant. It will be loaded from Firestore.
    let priceData = {};
    const coverageData = {
        // ... your coverage data remains unchanged
    };
    const supportData = {
        // ... your support data remains unchanged
    };
function uploadInitialPrices() {
    ...
    firebase.firestore().collection("settings").doc("pricing").set(initialPriceData)
        .then(() => {
            ...
            alert("Prices have been uploaded.");
        })
        .catch((error) => {
    ...
    async function loadPricesFromFirestore() {
        try {
            ...
            if (doc.exists) {
                ...
                // Now that prices are loaded, run the main calculation and UI setup
                runFullCalculation();
            } else {
    ...
    // This is your original systemCalculators object.
    const systemCalculators = {
        // ... your systemCalculators object remains unchanged
    };

    // The rest of your functions (populateSettingsModal, setupSettingsModal, etc.) remain here.
    // We are replacing the old loadPrices function.
...
async function generateDocument() {
  ...
  function initialize() {
   ...
   document.querySelectorAll('#number-of-networks, #max-antennas, #no-hardware-checkbox, #referral-fee-percent, #maintenance-percent, #customer-name, #survey-price, #quote-number, #total-service-antennas, #include-survey-checkbox').forEach(input => {
    ...
    // --- Initial Setup ---
   // Prices are now loaded after authentication
    setupSettingsModal();
