...
async function saveCoverageData(newCoverageData) {
    ...
    function updateUnitSellCellDisplay(cell, key) { const item = currentResults[key], displaySpan = cell.querySelector('.value-display'), isOverridden = item.unitSellOverride !== null, value = isOverridden ? item.unitSellOverride : (item.calculatedUnitSell || 0); displaySpan.textContent = `£${value.toFixed(2)}`; displaySpan.classList.toggle('overridden', isOverridden); }
